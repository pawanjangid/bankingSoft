<div class="row">
            <div class="form-group col-sm-6">
                        ...
                        <div class="col-sm-6">
                            <select name="plan_type" id="search_by" class="form-control" required="required" onchange="inputenable(this.value)">
                              ...
                              <option value="Member"><?php echo get_phrase('Member');?></option>
    ...
    <div class="col-sm-10" id="branch_wise" style="display: none;">
        ...
        <div class="col-sm-10" id="member_wise" style="display: none;">
      
            <label for="field-2" class="col-sm-2 control-label" style="font-size: 14px;color: green;"><?php echo get_phrase('Member_Associate_Code');?></label>
            <div class="col-sm-4">
                <input type="text" name="member_code" id="member_code" class="form-control" required="required">
            </div>
            <div class="col-sm-4">
                <button class="btn btn-primary" onclick="get_content_member()" value="Get Data">Get Data</button>
            </div>
        </div> 
    </div>
...
<div class="row">
    <div class="col-sm-10 offset-sm-1">
        <table class="table table-bordered">
            <thead>
                <tr style="font-size: 14px;color: green;font-weight: 600;">
                    <td>SL No</td>
                    <td>Memeber Code</td>
                    <td>Member Name</td>
                    <td>Gender</td>
...
<script type="text/javascript">
  ...
  xhttp.open("GET", "<?php echo base_url() .'index.php?admin/get_content_member/date/'; ?>" + first_date + "/" + last_date , true);
  xhttp.send();
}

function get_content_member() {
        document.getElementById('content_table').innerHTML = '';
  ...
  xhttp.open("GET", "<?php echo base_url() .'index.php?admin/get_content_member/member/'; ?>" + member_code , true);
  xhttp.send();
}
...
function get_content_branch() {
  ...
  xhttp.open("GET", "<?php echo base_url() .'index.php?admin/get_content_member/branch/'; ?>" + branch_id, true);
  xhttp.send();
}
...
function inputenable(selected) {
    if (selected == 'Date') {
        ...
        document.getElementById('member_wise').style.display = 'none';
    }
    if (selected == 'Branch') {
    ...
    if (selected == 'Member') {
        document.getElementById('branch_wise').style.display = 'none';
        document.getElementById('date_wise').style.display = 'none';
        document.getElementById('member_wise').style.display = 'block';
    }
}
