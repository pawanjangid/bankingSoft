...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('credit_manager_login') == 1)
         ...
         if ($this->session->userdata('branch_manager_login') == 1)
            redirect(base_url() . 'index.php?branch_manager/dashboard', 'refresh');
    ...
    function validate_login($email = '', $password = '') {
        ...
        $query = $this->db->get_where('agent', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('agent_login', '1');
            $this->session->set_userdata('agent_id', $row->agent_id);
            $this->session->set_userdata('login_user_id', $row->agent_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'agent');
            return 'success';
        }

        $query = $this->db->get_where('hr_manager', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('hr_manager_login', '1');
            $this->session->set_userdata('employee_id', $row->employee_id);
            $this->session->set_userdata('login_user_id', $row->employee_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'hr_manager');
            return 'success';
        }

        $query = $this->db->get_where('accountent', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('accountent_login', '1');
            $this->session->set_userdata('employee_id', $row->employee_id);
            $this->session->set_userdata('login_user_id', $row->employee_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'accountent');
            return 'success';
        }
        $query = $this->db->get_where('credit_manager', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('credit_manager_login', '1');
            $this->session->set_userdata('employee_id', $row->employee_id);
            $this->session->set_userdata('login_user_id', $row->employee_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'credit_manager');
            return 'success';
        }
        $query = $this->db->get_where('branch_manager', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('branch_manager_login', '1');
            $this->session->set_userdata('employee_id', $row->employee_id);
            $this->session->set_userdata('login_user_id', $row->employee_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'branch_manager');
            return 'success';
        }
        $query = $this->db->get_where('business_manager', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('business_manager_login', '1');
            $this->session->set_userdata('employee_id', $row->employee_id);
            $this->session->set_userdata('login_user_id', $row->employee_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'business_manager');
            return 'success';
        }
    ...
    function ajax_forgot_password()
    {
        ...
        if ($query->num_rows() > 0) 
        {
            $reset_account_type     =   'agent';
            $this->db->where('email' , $email);
            $this->db->update('employee' , array('password' => sha1($new_password)));
            $resp['status']         = 'true';
        }
